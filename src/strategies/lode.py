...
class LODECriterion(RegularizationMethod):
    def __init__(self):
        pass
    ...
    def __call__(
        ...
        out_in_curr = out_in[:, current_classes]
        oh_target_in = F.one_hot(target_in, num_classes=out_in.shape[1])
        target_in_oh_curr = oh_target_in[:, current_classes]

        loss_current_ce = cross_entropy_with_oh_targets(out_in_curr, target_in_oh_curr)
        loss_current_n = (-torch.logsumexp(out_in_curr, dim=1) + torch.logsumexp(out_in, dim=1)).mean()
        loss_buffer = F.cross_entropy(out_buffer, target_buffer)
        # return beta1*loss_current_ce + beta2*loss_current_n + loss_buffer
        return 0.5 * (beta1*loss_current_ce + beta2*loss_current_n + loss_buffer)  # 0.5 for consistency w/ ER-ACE
...
class LODE(SupervisedTemplate):
    def __init__(
        ...
        :param alpha: float : Determines weight on l_ce and l_n for the current
            databatch (default=0.05)
        ...
        self.lode_criterion = LODECriterion()
        self.C = C
        self.rho = rho
    ...
    def training_epoch(self, **kwargs):
        ...
        for self.mbatch in self.dataloader:
            ...
            if self.replay_loader is None:
                ...
                beta1 = self.C
                n_classes_curr = len(torch.unique(self.mb_y))
                n_classes_old = self.mb_output.size(-1) - n_classes_curr
                beta2 = self.rho * (n_classes_curr / n_classes_old if n_classes_old > 0 else 0.)
                self.loss += self.lode_criterion(self.mb_output, self.mb_y, self.mb_buffer_out,
                                                 self.mb_buffer_y, beta1, beta2)
