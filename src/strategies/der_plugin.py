...
class LogitClassBalancedBuffer(BalancedExemplarsBuffer):
    """
...
# TODO: Keep a model that is updated once every x iterations. Extend ReplayPlugin. For memory data,
# also compute the loss on crossentropy with previous model guesses.
class DERPlugin(SupervisedPlugin):
    """Also stores previous logits and replays them along with the targets (DER++)"""
    ...
    def __init__(
        ...
        self.update_every_exp = update_every_exp
    ...
    def after_training_exp(self, strategy, **kwargs):
        self.storage_policy.update(strategy, **kwargs)
        if strategy.clock.train_exp_counter % self.update_every_exp == 0:
    ...
    def before_backward(self, strategy, **kwargs):
        ...
        batch_x, batch_y, batch_tid = batch_x.to(device), batch_y.to(device), batch_tid.to(device)

        out_buffer = avalanche_forward(strategy.model, batch_x, batch_tid)
        with torch.no_grad():
            prev_logits = avalanche_forward(self.prev_model, batch_x, batch_tid)
        strategy.loss += self.alpha * F.mse_loss(out_buffer, prev_logits[:, :out_buffer.shape[1]])
        strategy.loss += self.beta * F.cross_entropy(out_buffer, batch_y)
