...
from src.toolkit.clear_benchmark import CLEAR
...
DS_SIZES = {
    ...
    "split_tinyimagenet_resized": (32, 32, 3),
...
def create_benchmark(
    ...
    *,
    ...
    if benchmark_name == "split_cifar100":
    ...
    elif benchmark_name == "split_tinyimagenet" or benchmark_name == "split_tinyimagenet_resized":
        if not use_transforms:
        ...
        if benchmark_name == "split_tinyimagenet_resized":
            train_transform.transforms.insert(1 if use_transforms else 0, transforms.Resize(32))
            eval_transform.transforms.insert(1 if use_transforms else 0, transforms.Resize(32))
    ...
    elif benchmark_name == "clear":
        if not use_transforms:
            train_transform = default_clear_eval_transform
            eval_transform = train_transform
        else:
            train_transform = default_clear_train_transform
            eval_transform = default_clear_eval_transform
