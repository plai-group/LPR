...
from src.strategies import (ER_ACE, LwFPlugin, OnlineICaRL, LODE,
                            OnlineICaRLLossPlugin, DERPlugin, LogitClassBalancedBuffer,
                            MeanEvaluation, LPRPlugin, GradClippingPlugin)
...
from src.toolkit.rep_drift_metric import RepresentationDriftMetric
from src.toolkit.slim_resnet18 import ResNet as SlimResNet
...
def create_strategy(
    ...
    if name == "naive":
        ...
        strategy = "Naive"
        specific_args = utils.extract_kwargs(
            ["mem_size", "batch_size_mem", "replay_weight"], strategy_kwargs
        )
        storage_policy = ClassBalancedBuffer(
            max_size=specific_args["mem_size"], adaptive_size=True
        )
        replay_plugin = SepReplayPlugin(**specific_args, storage_policy=storage_policy)
        plugins.append(replay_plugin)
        ...
        specific_args = utils.extract_kwargs(["alpha", "beta", "batch_size_mem"], strategy_kwargs)
        # storage_policy = LogitClassBalancedBuffer(
        #     max_size=strategy_kwargs.get("mem_size"), adaptive_size=True
        # )
        storage_policy = ClassBalancedBuffer(max_size=strategy_kwargs["mem_size"], adaptive_size=True)
        der_plugin = DERPlugin(**specific_args, storage_policy=storage_policy)
        plugins.append(der_plugin)
        ...
        strategy = "LODE"
        specific_args = utils.extract_kwargs(
            ["rho", "mem_size", "batch_size_mem"], strategy_kwargs
        )
        storage_policy = ClassBalancedBuffer(max_size=specific_args["mem_size"], adaptive_size=True)
        specific_args["storage_policy"] = storage_policy
        strategy_dict.update(specific_args)
        ...
        evaluator, parallel_eval_plugin = create_evaluator(
            logdir=new_logdir, wandb_logger=wandb_logger, **evaluation_kwargs
        )
    ...
    if strategy_dict["evaluator"] is None:
        ...
        rep_drift_kwargs = {**evaluation_kwargs.pop("rep_drift"), "data_dict": misc}
        if isinstance(model, SlimResNet):
            rep_drift_kwargs['selected_layer_names'] = ['layer1', 'layer2', 'layer3', 'layer4', 'linear.classifier']
        elif isinstance(model, ResNet):
            rep_drift_kwargs['selected_layer_names'] = ['layer1', 'layer2', 'layer3', 'layer4', 'fc.classifier']
        else:
            raise Exception("Unknown model type.")
        evaluator, parallel_eval_plugin = create_evaluator(
            logdir=logdir, wandb_logger=wandb_logger, rep_drift_kwargs=rep_drift_kwargs, **evaluation_kwargs
        )
        strategy_dict.update({"evaluator": evaluator})
    ...
    if strategy_kwargs["grad_clip"] is not None:
        clip_plugin = GradClippingPlugin(clipgrad=strategy_kwargs["grad_clip"])
        plugins.append(clip_plugin)

    if strategy_kwargs["ema_momentum"] is not None:
        ema_plugin = MeanEvaluation(momentum=strategy_kwargs.ema_momentum, update_at="iteration", replace=True)
        plugins.append(ema_plugin)

    if strategy_kwargs["lpr"]:
        if name != "der":  # For DER, we directly access its storage policy from the strategy object
            assert storage_policy is not None
        lpr_plugin = LPRPlugin(lpr_kwargs=strategy_kwargs["lpr_kwargs"], storage_policy=storage_policy)
        plugins.append(lpr_plugin)
...
def get_loggers(loggers_list, logdir, prefix="logs", wandb_logger=None):
    loggers = []
...
def create_evaluator(
    ...
    **parallel_eval_kwargs,
    ...
    loggers_strategy = get_loggers(loggers_strategy, logdir, prefix="logs", wandb_logger=wandb_logger)
    if rep_drift_kwargs.pop("enable"):
        strategy_metrics.append(RepresentationDriftMetric(**rep_drift_kwargs))
    ...
    if parallel_evaluation:
        ...
        loggers_parallel = get_loggers(
            loggers_parallel, logdir, prefix="logs_continual", wandb_logger=wandb_logger
        )
